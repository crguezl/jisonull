(function(global,factory){typeof exports==='object'&&typeof module!=='undefined'?module.exports=factory(require('@gerhobbelt/xregexp'),require('@gerhobbelt/json5'),require('fs'),require('path'),require('@crguezl/xregexp'),require('recast'),require('@babel/core'),require('assert'),require('@crguezl/json5')):typeof define==='function'&&define.amd?define(['@gerhobbelt/xregexp','@gerhobbelt/json5','fs','path','@crguezl/xregexp','recast','@babel/core','assert','@crguezl/json5'],factory):(global=global||self,global['ebnf-parser']=factory(global.XRegExp,global.JSON5,global.fs,global.path$1,global.XRegExp$1,global.recast,global.babel,global.assert$1,global.JSON5$1));})(this,function(XRegExp,JSON5,fs,path$1,XRegExp$1,recast,babel,assert$1,JSON5$1){'use strict';XRegExp=XRegExp&&XRegExp.hasOwnProperty('default')?XRegExp['default']:XRegExp;JSON5=JSON5&&JSON5.hasOwnProperty('default')?JSON5['default']:JSON5;fs=fs&&fs.hasOwnProperty('default')?fs['default']:fs;path$1=path$1&&path$1.hasOwnProperty('default')?path$1['default']:path$1;XRegExp$1=XRegExp$1&&XRegExp$1.hasOwnProperty('default')?XRegExp$1['default']:XRegExp$1;recast=recast&&recast.hasOwnProperty('default')?recast['default']:recast;assert$1=assert$1&&assert$1.hasOwnProperty('default')?assert$1['default']:assert$1;JSON5$1=JSON5$1&&JSON5$1.hasOwnProperty('default')?JSON5$1['default']:JSON5$1;// Return TRUE if `src` starts with `searchString`. 
...
set=set.replace(/\s+/g,'');assert$1(set.length>=1);let lsidx=0;let lsfreq=Infinity;for(let i=0,l=set.length;i<l;i++){let ch=set[i];let freq=countOccurrences(ch);if(freq===0){return ch;}if(freq<lsfreq){lsfreq=freq;lsidx=i;}}return set[lsidx];}const escCharSet="ဩ ℹ இ ண ஐ Ϟ ല ઊ";// Currently we only need 7 rows of typeIdCharSets. The other rows are commented out but available for future use:
...
//   
//   -> (1) start-#
tokenDirectIdentifierStart:escChar+typeIdChar[0],tokenDirectIdentifierRe:new XRegExp$1(`#(${ID_REGEX_BASE})#`,'g'),// - alias/token value references, e.g. `$token`, `$2`
// 
//   -> $ is an accepted starter, so no encoding required
// - result value reference `$$`
// 
//   -> $ is an accepted starter, so no encoding required
tokenValueReferenceStart:'$',tokenValueReferenceRe:new XRegExp$1(`$(${ID_REGEX_BASE})|$([0-9]+)`,'g'),// - alias/token location reference, e.g. `@token`, `@2`
// 
//   -> (6) single-@
// - result location reference `@$`
// 
//   -> (6) single-@
tokenLocationStart:escChar+typeIdChar[1],tokenLocationRe:new XRegExp$1(`@(${ID_REGEX_BASE})|@([0-9]+)`,'g'),// - alias/token id numbers, e.g. `#token`, `#2`
// 
//   -> (3) single-#
// - rule id number `#$`
// 
//   -> (3) single-#
tokenIdentifierStart:escChar+typeIdChar[2],tokenIdentifierRe:new XRegExp$1(`#(${ID_REGEX_BASE})|#([0-9]+)`,'g'),// - alias/token stack indexes, e.g. `##token`, `##2`
// 
//   -> (4) double-#
// - result stack index `##$`
// 
//   -> (4) double-#
tokenStackIndexStart:escChar+typeIdChar[3],tokenStackIndexRe:new XRegExp$1(`##(${ID_REGEX_BASE})|##([0-9]+)`,'g'),// - 'negative index' value references, e.g. `$-2`
// 
//   -> (8) single-negative-$
tokenNegativeValueReferenceStart:escChar+typeIdChar[4],tokenValueReferenceRe:new XRegExp$1(`$-([0-9]+)`,'g'),// - 'negative index' location reference, e.g. `@-2`
// 
//   -> (7) single-negative-@
tokenNegativeLocationStart:escChar+typeIdChar[5],tokenNegativeLocationRe:new XRegExp$1(`@-([0-9]+)`,'g'),// - 'negative index' stack indexes, e.g. `##-2`
// 
//   -> (5) double-negative-#
tokenNegativeStackIndexStart:escChar+typeIdChar[6],tokenNegativeStackIndexRe:new XRegExp$1(`#-([0-9]+)`,'g'),// combined regex for encoding direction
tokenDetect4EncodeRe:new XRegExp$1(`([^$@#${IN_ID_CHARSET}])([$@#]|##)(${ID_REGEX_BASE}|[$]|-?[0-9]+)(#?)(?![$@#${IN_ID_CHARSET}])`,'g'),// combined regex for decoding direction
tokenDetect4DecodeRe:new XRegExp$1(`([^$${IN_ID_CHARSET}])(${escChar}[${typeIdChar.slice(0,7).join('')}])(${ID_REGEX_BASE}|[$]|[0-9]+)(?![$@#${IN_ID_CHARSET}])`,'g'),encode:function encodeJisonTokens(src,locationOffsetSpec){let re=this.tokenDetect4EncodeRe;// reset regex
...
return src.substring(1);}};}function parseCodeChunkToAST(src,options){let s=options.mapper4JisonGrammarIdentifiers.encode(src,options.mapperErrorReporter);let ast=recast.parse(s);return ast;}function compileCodeToES5(src,options){options=Object.assign({},{ast:true,code:true,sourceMaps:true,comments:true,filename:'compileCodeToES5.js',sourceFileName:'compileCodeToES5.js',sourceRoot:'.',sourceType:'module',babelrc:false,ignore:["node_modules/**/*.js"],compact:false,retainLines:false,presets:[["@babel/preset-env",{targets:{browsers:["last 2 versions"],node:"8.0"}}]]},options);return babel.transformSync(src,options);// => { code, map, ast }
}function prettyPrintAST(ast,options){const defaultOptions={tabWidth:2,quote:'single',arrowParensAlways:true,// Do not reuse whitespace (or anything else, for that matter)
// when printing generically.
reuseWhitespace:false};let s=recast.prettyPrint(ast,defaultOptions);let new_src=s.code;new_src=new_src.replace(/\r\n|\n|\r/g,'\n');// platform dependent EOL fixup
// backpatch possible jison variables extant in the prettified code:
let dst=options.mapper4JisonGrammarIdentifiers.decode(new_src,options.mapperErrorReporter);return dst;}// validate the given JISON+JavaScript snippet: does it compile?
// 
...
var alias_re=new XRegExp$1(`\\[${ID_REGEX_BASE$1}\\]`);var term_re=new XRegExp$1(`^${ID_REGEX_BASE$1}$`);// and collect the PERMITTED aliases: the names of the terms and all the remaining aliases
...
//
var nameref_re=new XRegExp$1(`(?:[$@]|##)${ID_REGEX_BASE$1}`,'g');var named_spots=nameref_re.exec(action);var numbered_spots=action.match(/(?:[$@]|##)[0-9]+\b/g);var max_term_index=list.terms.length;// loop through the XRegExp alias regex matches in `action`
...
var srcCode=trimActionCode$1(yyvstack[yysp-1]+yyvstack[yysp],yyvstack[yysp-2]);if(srcCode){var rv=checkActionBlock$1(srcCode,yylstack[yysp-1],yy);if(rv){yyparser.yyError(rmCommonWS$1`
...
name=lst[0][0];}else if(len<=1){yyparser.yyError(rmCommonWS$1`
        ...
        `);}else{yyparser.yyError(rmCommonWS$1`
            You did specify too many attributes for the '%code' initialization code statement, which must have the format:
              ...
              Erroneous code:
            ${yylexer.prettyPrintRange(yylstack[yysp-4],yylstack[yysp-5])}
        `);}var srcCode=trimActionCode$1(yyvstack[yysp-2]+yyvstack[yysp-1],yyvstack[yysp-3]);var rv=checkActionBlock$1(srcCode,yylstack[yysp-2],yy);if(rv){yyparser.yyError(rmCommonWS$1`
...
this._$=yyparser.yyMergeLocationInfo(yysp-4,yysp);// END of default action (generated by JISON mode classic/merge :: 5,VT,VA,VU,-,LT,LA,-,-)
this.$=[yyvstack[yysp-4]];var srcCode=trimActionCode$1(yyvstack[yysp-1]+yyvstack[yysp],yyvstack[yysp-2]);if(srcCode){var rv=checkActionBlock$1(srcCode,yylstack[yysp-1],yy);if(rv){yyparser.yyError(rmCommonWS$1`
...
this._$=yyparser.yyMergeLocationInfo(yysp-3,yysp);// END of default action (generated by JISON mode classic/merge :: 4,VT,VA,VU,-,LT,LA,-,-)
this.$=[[]];var srcCode=trimActionCode$1(yyvstack[yysp-1]+yyvstack[yysp],yyvstack[yysp-2]);if(srcCode){var rv=checkActionBlock$1(srcCode,yylstack[yysp-1],yy);if(rv){yyparser.yyError(rmCommonWS$1`
...
this._$=yyparser.yyMergeLocationInfo(yysp-3,yysp);// END of default action (generated by JISON mode classic/merge :: 4,VT,VA,VU,-,LT,LA,-,-)
var srcCode=trimActionCode$1(yyvstack[yysp-1]+yyvstack[yysp],yyvstack[yysp-2]);var rv=checkActionBlock$1(srcCode,yylstack[yysp-1],yy);if(rv){yyparser.yyError(rmCommonWS$1`
...
this._$=yylstack[yysp];// END of default action (generated by JISON mode classic/merge :: 1,VT,VA,VU,-,LT,LA,-,-)
this.$=JSON5.parse(yyvstack[yysp]);break;case 137:/*! Production::    option_value : OPTION_VALUE */ // default action (generated by JISON mode classic/merge :: 1,VT,VA,VU,-,LT,LA,-,-):
this._$=yylstack[yysp];// END of default action (generated by JISON mode classic/merge :: 1,VT,VA,VU,-,LT,LA,-,-)
...
var fileContent=fs.readFileSync(path,{encoding:'utf-8'});var srcCode=trimActionCode$1(fileContent);if(srcCode){var rv=checkActionBlock$1(srcCode,this._$,yy);if(rv){yyparser.yyError(rmCommonWS$1`
...
};return 29;case 84:/*! Conditions:: code */ /*! Rule::       (?:[^%{BR}][^{BR}]*{BR}+)+ */return 60;// shortcut to grab a large bite at once when we're sure not to encounter any `%include` in there at start-of-line.  
...
// If so, use that one instead of the less palatable token set.
if(!do_not_describe&&this.state_descriptions_&&this.state_descriptions_[state]){return[this.state_descriptions_[state]];}for(var p in this.table[state]){p=+p;if(p!==TERROR){var d=do_not_describe?p:this.describeSymbol(p);if(d&&!check[d]){tokenset.push(d);check[d]=true;// Mark this token description as already mentioned to prevent outputting duplicate entries.
}}}return tokenset;},productions_:bp$2({pop:u$2([56,s$2,[57,5],58,59,59,s$2,[60,21],s$2,[61,8,1],s$2,[68,13],s$2,[69,5],70,70,s$2,[71,5],s$2,[72,7],73,73,74,75,75,s$2,[76,5],77,77,s$2,[78,18],79,80,80,81,81,82,82,83,84,84,s$2,[85,3],s$2,[86,4],87,87,88,88,s$2,[89,3],s$2,[90,3],s$2,[91,5],92,92]),rule:u$2([4,3,3,2,2,0,0,2,0,3,2,3,2,c$2,[4,3],1,c$2,[5,3],c$2,[3,3],1,3,2,6,4,2,s$2,[1,8],2,2,4,2,3,4,c$2,[25,3],s$2,[2,4],0,2,4,c$2,[53,4],0,c$2,[6,5],c$2,[19,7],4,3,1,1,c$2,[66,3],c$2,[49,3],c$2,[58,3],s$2,[3,3],s$2,[2,5],c$2,[12,3],s$2,[1,7],c$2,[17,3],c$2,[9,7],c$2,[8,3],c$2,[13,8],c$2,[35,5],c$2,[13,5],3,2])}),performAction:function parser__PerformAction(yyloc,yystate/* action[1] */,yysp,yyvstack,yylstack){/* this == yyval */ // the JS engine itself can go and remove these statements when `yy` turns out to be unused in any action code!
...
// macros here:
if(XRegExp$1._getUnicodeProperty(yyvstack[yysp-2])){// Work-around so that you can use `\p{ascii}` for a XRegExp slug, a.k.a.
...
this._$=yylstack[yysp];// END of default action (generated by JISON mode classic/merge :: 1,VT,VA,VU,-,LT,LA,-,-)
if(XRegExp$1._getUnicodeProperty(yyvstack[yysp].replace(/[{}]/g,''))&&yyvstack[yysp].toUpperCase()!==yyvstack[yysp]){// treat this as part of an XRegExp `\p{...}` Unicode 'General Category' Property cf. http://unicode.org/reports/tr18/#Categories
...
this._$=yylstack[yysp];// END of default action (generated by JISON mode classic/merge :: 1,VT,VA,VU,-,LT,LA,-,-)
this.$=JSON5$1.parse(yyvstack[yysp]);break;case 121:/*! Production::    option_value : OPTION_VALUE */ // default action (generated by JISON mode classic/merge :: 1,VT,VA,VU,-,LT,LA,-,-):
this._$=yylstack[yysp];// END of default action (generated by JISON mode classic/merge :: 1,VT,VA,VU,-,LT,LA,-,-)
this.$=parseValue$1(yyvstack[yysp]);break;case 122:/*! Production::    epilogue : "%%" */ // default action (generated by JISON mode classic/merge :: 1,VT,VA,VU,-,LT,LA,-,-):
this._$=yylstack[yysp];// END of default action (generated by JISON mode classic/merge :: 1,VT,VA,VU,-,LT,LA,-,-)
...
path=lst[0][0];}else if(len<=1){yyparser.yyError(rmCommonWS$2`
        ...
        `);}else{yyparser.yyError(rmCommonWS$2`
            ...
            ${$error.errStr}
        `);}// **Aside**: And no, we don't support nested '%include'!
var fileContent=fs.readFileSync(path,{encoding:'utf-8'});var srcCode=trimActionCode$2(fileContent);if(srcCode){var rv=checkActionBlock$2(srcCode,this._$,yy);if(rv){yyparser.yyError(rmCommonWS$2`
                The source code included from file '${path}' does not compile: ${rv}
...
this.unput(yy_.yytext);return 26;case 81:/*! Conditions:: INITIAL rules code */ /*! Rule::       %{NAME}([^\r\n]*) */ /* ignore unrecognized decl */this.warn(rmCommonWS`
                                                ...
                                                `+this.prettyPrintRange(yy_.yylloc));yy_.yytext={name:this.matches[1],// {NAME}  
value:this.matches[2].trim()// optional value/parameters 
};return 28;case 82:/*! Conditions:: rules macro INITIAL */ /*! Rule::       %% */this.pushState('rules');return 19;case 90:/*! Conditions:: set */ /*! Rule::       \] */this.popState();return 47;case 91:/*! Conditions:: code */ /*! Rule::       (?:[^%{BR}][^{BR}]*{BR}+)+ */return 55;// shortcut to grab a large bite at once when we're sure not to encounter any `%include` in there at start-of-line.  
