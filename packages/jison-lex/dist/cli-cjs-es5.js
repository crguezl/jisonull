...
var rv=false;for(var i=0,len=rulesSpecSize;i<len;i++){var lastEditedRuleSpec;rv=test_me(function(){assert__default['default'](Array.isArray(opts.lex_rule_dictionary.rules));assert__default['default'](opts.lex_rule_dictionary.rules.length===rulesSpecSize);// opts.conditions = [];
// opts.rules = [];
...
var rule=opts.lex_rule_dictionary.rules[j];assert__default['default'](Array.isArray(rule));assert__default['default'](rule.length===2||rule.length===3);rule.pop();rule.push('{ /* nada */ }');lastEditedRuleSpec=rule;}},function(){return'Your lexer rule "'+lastEditedRuleSpec[0]+'" action code block is botched?';},ex,null);if(rv){break;}}if(!rv){test_me(function(){opts.conditions=[];opts.lex_rule_dictionary.rules=[];opts.performAction='null';// opts.options = {};
