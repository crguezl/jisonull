...
'use strict';function _interopDefault(ex){return ex&&typeof ex==='object'&&'default'in ex?ex['default']:ex;}var fs=_interopDefault(require('fs'));var path$1=_interopDefault(require('path'));var commander=require('commander');var XRegExp=_interopDefault(require('@gerhobbelt/xregexp'));var recast=_interopDefault(require('recast'));var babel=require('@babel/core');var assert$1=_interopDefault(require('assert'));var XRegExp$1=_interopDefault(require('@crguezl/xregexp'));var JSON5=_interopDefault(require('@crguezl/json5'));// Return TRUE if `src` starts with `searchString`. 
...
return retval;},yyError:1};parser.originalParseError=parser.parseError;parser.originalQuoteName=parser.quoteName;/* lexer generated by jison-lex 0.6.1-215 */ /*
...
var rmCommonWS$2=helpers.rmCommonWS;var mkIdentifier$2=helpers.mkIdentifier;var code_exec$1=helpers.exec;var version='0.6.5-223';// require('./package.json').version;
...
// it in a regex; that way we can easily validate whether macro X is fit to be used
// inside a regex set:
try{var re;re=new XRegExp$1(s);re.test(s[0]);}catch(ex){// make sure we produce a regex expression which will fail badly when it is used
// in actual code:
...
// macros here:
if(XRegExp$1._getUnicodeProperty(k)){// Work-around so that you can use `\p{ascii}` for a XRegExp slug, a.k.a.
// Unicode 'General Category' Property cf. http://unicode.org/reports/tr18/#Categories,
// while using `\p{ASCII}` as a *macro expansion* of the `ASCII`
// macro:
...
// HACK:
if(!opt.mapper4JisonGrammarIdentifiers){console.error("WARNING: auto-setup of jison Unicode ID mapper!");opt.mapper4JisonGrammarIdentifiers=helpers.generateMapper4JisonGrammarIdentifiers(src);}var ast=helpers.parseCodeChunkToAST(src,opt);var new_src=helpers.prettyPrintAST(ast,opt);// inject analysis report now:
...
throw new Error(msg);}}opt.moduleName='lexer';}prepExportStructures(opt);return opt;}function generateModule(opt){opt=prepareOptions(opt);var modIncSrc=opt.moduleInclude?opt.moduleInclude+';':'';var src=rmCommonWS$2`
    ...
    `;src=stripUnusedLexerCode(src,opt);opt.exportSourceCode.all=src;return src;}RegExpLexer.generate=generate;RegExpLexer.version=version;RegExpLexer.defaultJisonLexOptions=defaultJisonLexOptions;RegExpLexer.mkStdOptions=mkStdOptions;RegExpLexer.camelCase=helpers.camelCase;RegExpLexer.mkIdentifier=mkIdentifier$2;RegExpLexer.autodetectAndConvertToJSONformat=autodetectAndConvertToJSONformat;var mkIdentifier$3=helpers.mkIdentifier;var version$1='0.6.5-223';// require('./package.json').version;
function getCommandlineOptions(){var program=new commander.Command();var opts={file:null,json:false,outfile:null,debug:false,dumpSourceCodeOnFailure:true,throwErrorOnCompileFailure:true,reportStats:false,moduleType:'commonjs',moduleName:null,main:false,moduleMain:null};program.name('jison-lex').version(version$1).strictOption(false).argument('[file]','file containing a lexical grammar.').option('-j, --json','jison will expect a grammar in either JSON/JSON5 or JISON format: the precise format is autodetected.').option('-o, --outfile <FILE>','Filepath and base module name of the generated parser. When terminated with a "/" (dir separator) it is treated as the destination directory where the generated output will be stored.').option('-t, --debug','Debug mode.').option('--dump-sourcecode-on-failure','Dump the generated source code to a special named file when the internal generator tests fail.').option('--throw-on-compile-failure','Throw an exception when the generated source code fails to compile in the JavaScript engine.').option('-I, --info','Report some statistics about the generated parser.').option('-m, --module-type <TYPE>','The type of module to generate (commonjs, es, js)','commonjs').option('-n, --module-name <NAME>','The name of the generated parser object, namespace supported.').option('-x, --main','Include .main() entry point in generated commonjs module.').option('-y, --module-main <NAME>','The main module function definition.').action(function(file){opts.file=file;});var parsed=program.parse();var cmdOpts=parsed.opts();// Map commander options to our internal format
opts.json=cmdOpts.json||false;opts.outfile=cmdOpts.outfile||null;opts.debug=cmdOpts.debug||false;opts.dumpSourceCodeOnFailure=cmdOpts.dumpSourcecodeOnFailure!==false;opts.throwErrorOnCompileFailure=cmdOpts.throwOnCompileFailure!==false;opts.reportStats=cmdOpts.info||false;opts.moduleType=cmdOpts.moduleType;opts.moduleName=cmdOpts.moduleName||null;opts.main=cmdOpts.main||false;opts.moduleMain=cmdOpts.moduleMain||null;if(opts.debug){console.log("JISON-LEX CLI options:\n",opts);}return opts;}function cliMain(opts){opts=RegExpLexer.mkStdOptions(opts);function isDirectory(fp){try{return fs.lstatSync(fp).isDirectory();}catch(e){return false;}}function mkdirp(fp){if(!fp||fp==='.'||fp.length===0){return false;}try{fs.mkdirSync(fp);return true;}catch(e){if(e.code==='ENOENT'){var parent=path$1.dirname(fp);// Did we hit the root directory by now? If so, abort!
...
fs.mkdirSync(fp);return true;}catch(e){return false;}}}}return false;}function processInputFile(){var original_cwd=process.cwd();var raw=fs.readFileSync(path$1.normalize(opts.file),'utf8');// making best guess at json mode
opts.json=path$1.extname(opts.file)==='.json'||opts.json;// When only the directory part of the output path was specified, then we
// do NOT have the target module name in there as well!
var outpath=opts.outfile;if(typeof outpath==='string'){if(/[\\\/]$/.test(outpath)||isDirectory(outpath)){opts.outfile=null;outpath=outpath.replace(/[\\\/]$/,'');}else{outpath=path$1.dirname(outpath);}}else{outpath=null;}if(outpath&&outpath.length>0){outpath+='/';}else{outpath='';}// setting output file name and module name based on input file name
// if they aren't specified.
var name=path$1.basename(opts.outfile||opts.file);// get the base name (i.e. the file name without extension)
// i.e. strip off only the extension and keep any other dots in the filename
name=path$1.basename(name,path$1.extname(name));opts.outfile=opts.outfile||outpath+name+'.js';if(!opts.moduleName&&name){opts.moduleName=opts.defaultModuleName=mkIdentifier$3(name);}// Change CWD to the directory where the source grammar resides: this helps us properly
// %include any files mentioned in the grammar with relative paths:
var new_cwd=path$1.dirname(path$1.normalize(opts.file));process.chdir(new_cwd);var lexer=cli.generateLexerString(raw,opts);// and change back to the CWD we started out with:
process.chdir(original_cwd);opts.outfile=path$1.normalize(opts.outfile);mkdirp(path$1.dirname(opts.outfile));fs.writeFileSync(opts.outfile,lexer);console.log('JISON-LEX output for module ['+opts.moduleName+'] has been written to file:',opts.outfile);}function readin(cb){var stdin=process.openStdin(),data='';stdin.setEncoding('utf8');stdin.addListener('data',function(chunk){data+=chunk;});stdin.addListener('end',function(){cb(data);});}function processStdin(){readin(function processStdinReadInCallback(raw){console.log(cli.generateLexerString(raw,opts));});}// if an input file wasn't given, assume input on stdin
